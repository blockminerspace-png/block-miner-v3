...
try {
    ...
    if (-not $skipEnvUpload) {
        ...
        # ZerAds PTC / callback — same merge rules as VITE_* (keys from deploy.secrets.local).
        $zeradsMap = @{}
        foreach ($key in $deploySecrets.Keys) {
            if ($key -match '^ZERADS_' -and $deploySecrets[$key] -and $deploySecrets[$key].Trim()) {
                $zeradsMap[$key] = $deploySecrets[$key].Trim()
            }
        }
        if ($zeradsMap.Count -gt 0) {
